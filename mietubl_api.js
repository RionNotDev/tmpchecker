...
module.exports = async (req, res) => {
    ...
    const { api_action, model, panel, query } = body;
    if (api_action === 'search') {
        ...
        try {
            ...
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ success: false, error: e.message, matches: [], groups: [] }));
        }
    } else if (api_action === 'fetch') {
        try {
            const html = await fetchPage(query);
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ success: true, html }));
        } catch (e) {
            res.setHeader('Content-Type', 'application/json');
        ...
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ success: false, error: 'Invalid API action' }));
    }
};
